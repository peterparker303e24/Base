import { ethers, keccak256 } from "../libs/ethers.js";
import {
    loadHeader,
    prefixHexBytes,
    replaceClass,
...
keyRevealButton.addEventListener("click", () => {
    if (isKeyRevealOn) {
        keyRevealButton.innerHTML = "OFF";
    } else {
        keyRevealButton.innerHTML = "ON";
    }
...
async function tryMatchFile(zipHash) {
    ...
    errorText.innerHTML = `[X] ERROR: Requirement.zip file not found at any user `
...
async function validateTaskFile(arrayBuffer) {
    ...
    for (let i = 0; i < idArray.length; i++) {
        ...
        if (fileHash !== requirementsHashes[idArray[i]]) {
            fileError.innerHTML = `[X] ERROR: Requirement${idArray[i]} `
...
function updateAddHashButton() {
    ...
    if (!fileCrossChecked) {
        ...
        errorText.innerHTML = `Invalid 32 bytes hash value`;
